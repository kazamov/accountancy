...
describe('LegalityDialogComponent', () => {
	...
	it('should decline action button be enabled by default', () => {
		const declineActionButton: DebugElement[] = getButtonById(
			fixture,
			'decline-action'
		);
	...
	it('should decline action button has warn color and matDialogClose attribute and is raised', () => {
		...
		const declineActionButtonAttributes = declineActionButton.map(
		...
		expect(
			declineActionButtonAttributes['mat-raised-button']
		).toBeDefined();
	});

	it('should confirm action button be disabled by default', () => {
		const confirmActionButton: DebugElement[] = getButtonById(
			fixture,
			'confirm-action'
		);
		const confirmActionButtonAttributes = confirmActionButton.map(
			button => button.attributes
		)[0];
		expect(confirmActionButtonAttributes['ng-reflect-disabled']).toMatch(
			'false'
		);
	});
	...
	it('should confirm action button has primary color and is raised', () => {
		...
		expect(
			confirmActionButtonAttributes['mat-raised-button']
		).toBeDefined();
	});

	it('should enable confirm button when only tosURL is provided and checkTOS is checked', () => {
		const disableConfirmActionButtonSpy = jest.spyOn(
			component,
	...
	it(
		'should enable confirm button when only tosUrl and privacyPolicyUrl is provided as well as' +
