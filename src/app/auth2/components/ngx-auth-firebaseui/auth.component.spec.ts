...
describe('AuthComponent', function() {
		...
		TestBed.configureTestingModule({
			...
			providers: [
				...
				{ provide: AngularFireAuth, useValue: AngularFireAuthStub },
				{
					...
					useValue: NgxAuthFirebaseUIConfigToken
				}
			],
			declarations: [EmailConfirmationComponent, AuthComponent],
	...
	it('should not call updateAuthSnackbarMessages when the messages input not changed', () => {
		console.log(
			...
			testBedService.messageOnAuthSuccess
		);

		const updateAuthSnackbarMessagesSpy = jest.spyOn(
			component,
	...
	it('should update the snackbar messages on component changes', () => {
		...
		component.messageOnAuthSuccess =
			'Here we go! The authentication was successful!';
		component.messageOnAuthError =
			'Oop! Something went wrong! Please retry again!';

		component.ngOnChanges({
			messageOnAuthSuccess: new SimpleChange(
				null,
				component.messageOnAuthSuccess,
				true
			),
			messageOnAuthError: new SimpleChange(
				null,
				component.messageOnAuthError,
				true
			)
		...
		expect(component.authProcess.messageOnAuthSuccess).toEqual(
			component.messageOnAuthSuccess
		);
		expect(component.authProcess.messageOnAuthError).toBeDefined();
		expect(component.authProcess.messageOnAuthError).toEqual(
			component.messageOnAuthError
		);
	});
});
